...
/** A workout estimate returned by the workout route. */
export interface WorkoutEstimate {
  activity: string;
  /** Minutes, if the user stated/implied a duration (else 0). */
  durationMin: number;
  /** Estimated calories burned. */
  calories: number;
  /** Short note on assumptions (intensity, body weight, etc.). */
  note: string;
}
...
const WORKOUT_SCHEMA = {
  ...
  required: ["activity", "durationMin", "calories", "note"],
  propertyOrdering: ["activity", "durationMin", "calories", "note"],
} as const;

const WORKOUT_INSTRUCTION = `You estimate calories burned from a described workout for a calorie tracker.
- "activity" is a short clean name (e.g. "Running", "Cycling", "Yoga", "Strength training").
- "durationMin" is the minutes exercised (0 if not stated).
- "calories" is the estimated calories burned for a typical adult (~70 kg) at the stated/implied intensity.
- Put assumptions (intensity, assumed weight/duration) in "note". Keep estimates realistic.`;

/** Estimate calories burned for a free-text workout description. */
export async function estimateWorkout(description: string): Promise<WorkoutEstimate> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new GeminiError("GEMINI_API_KEY is not set in the environment.", 500);

  const body = {
    systemInstruction: { parts: [{ text: WORKOUT_INSTRUCTION }] },
    contents: [{ role: "user", parts: [{ text: `Estimate calories burned: ${description}` }] }],
    generationConfig: {
  ...
  if (!res.ok) {
    ...
    throw new GeminiError(`Gemini request failed (${res.status}): ${truncate(detail)}`);
  }
  ...
  return {
    activity: String(parsed.activity ?? "Workout").trim() || "Workout",
